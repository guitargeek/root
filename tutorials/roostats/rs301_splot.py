...
def AddModel(object):
    ...
    # make the 2-d model
    qcdModel = ROOT.RooProdPdf("qcdModel", "2-d model for QCD", ROOT.RooArgSet(qcdMassModel, qcdIsolationModel))
    ...
    try:
        object.Import(massModel, RecycleConflictNodes=True)
...
def AddData(object):
    ...
    data = ROOT.RooDataSet(model.generate((invMass, isolation)))
...
def DoSPlot(object):
    ...
    for i in range(10):
        print(
            "z Weight for event ", i, ROOT.std.right, ROOT.std.setw(12), sData.GetSWeight(i, "zYield"), "  qcd Weight"
        )
        print(
            ROOT.std.setw(12),
            sData.GetSWeight(i, "qcdYield"),
            "  Total Weight",
            ROOT.std.setw(12),
            sData.GetSumOfEventSWeight(i),
        )
...
def MakePlots(object):
    ...
    # """
    ##info for debuggin :
    ## ROOT.nullptr doesn't work for : const ROOT.RooFormulaVar & cutvar
    ##                                 const char* wgtVarName = nullptr
    ##                                 const char* cuts = nullptr
    ## not even using hex(id(nullptr))
    ## improve pythonzation...
    ## ROOT.RooDataSet::RooDataSet(RooStringView name, ROOT.RooStringView title, ROOT.RooDataSet* data, const ROOT.RooArgSet& vars, const ROOT.RooFormulaVar&     cutVar, const char* wgtVarName = nullptr)
    ## ROOT.RooDataSet::RooDataSet(RooStringView name, ROOT.RooStringView title, ROOT.RooDataSet* data, const ROOT.RooArgSet& vars, const char* cuts = nu    llptr, const char* wgtVarName = nullptr)
    # """
    cutVar0 = ROOT.RooFormulaVar()  # continues at ...
    # for the purpose of this tutorial we don't use cuts.
    # however, it is possible to implement them as shown below:
    zYield = object.var("zYield")
    qcdYield = object.var("qcdYield")
    ral = ROOT.RooArgList(zYield, qcdYield)
    cutVar = ROOT.RooFormulaVar("zYield < 0 and qcdYield < 0.5", "Cuts", " @0 < 0 && @1 < 0.5", ral, True)
    # ... continues here
    dataw_qcd = ROOT.RooDataSet(data.GetName(), data.GetTitle(), data, data.get(), cutVar0, "qcdYield_sw")
    dataw_z = ROOT.RooDataSet(data.GetName(), data.GetTitle(), data, data.get(), cutVar0, "zYield_sw")
    ...
    dataw_z.plotOn(frame2, DataError=ROOT.RooAbsData.SumW2)
    ...
    dataw_qcd.plotOn(frame3, DataError=ROOT.RooAbsData.SumW2)
