...
TEST(RooSimultaneous, MixtureCompilationConditionalObs)
{
   ...
   RooRealVar m1{"m1", "m1", 0., -3., 3.};
   RooRealVar m2{"m2", "m2", 1., -3., 3.};
   RooRealVar s{"s", "s", 1.0, 0.1, 10.};
   // The width of the Gaussians scales with the conditional observable y.
   RooFormulaVar sigma{"sigma", "s * y", {s, y}};
   RooGaussian g1{"g1", "g1", x, m1, sigma};
   RooGaussian g2{"g2", "g2", x, m2, sigma};
   ...
   RooUniform yPdf{"yPdf", "yPdf", y};
   RooProdPdf gen1{"gen1", "gen1", yPdf, Conditional(g1, x)};
   RooProdPdf gen2{"gen2", "gen2", yPdf, Conditional(g2, x)};
   std::unique_ptr<RooDataSet> data1{gen1.generate({x, y}, 600)};
   std::unique_ptr<RooDataSet> data2{gen2.generate({x, y}, 800)};
   RooDataSet combData{
      "combData", "combData", {x, y}, Index(sample), Import({{"one", data1.get()}, {"two", data2.get()}})};

   auto setParams = [&](bool alternative) {
      s.setVal(alternative ? 1.5 : 1.0);
      m1.setVal(alternative ? 0.5 : 0.0);
   };
...
TEST(RooSimultaneous, MixtureCompilationPartiallyExtended)
{
   using namespace RooFit;
   ...
   RooRealVar n1{"n1", "n1", 1000., 0., 100000.};
   RooExtendPdf e1{"e1", "e1", g1, n1};
   // The second channel pdf is not extendable.
   RooGaussian g2{"g2", "g2", x2, m2, sigma};

   RooCategory sample{"sample", "sample", {{"one", 0}, {"two", 1}}};
   RooSimultaneous simPdf{"simPdf", "simPdf", {{"one", &e1}, {"two", &g2}}, sample};
...
/// Validate the binned-likelihood mixture compilation for channels without
/// RooBinWidthFunctions (like models from before ROOT 6.26): their compiled
/// values are probability densities, and the concatenated likelihood
/// multiplies each row with the bin volume provided by the mixture.
TEST(RooSimultaneous, BinnedMixtureCompilationNoBinWidthFunctions)
{
   using namespace RooFit;
   ...
   RooRealVar mu1{"mu_1", "mu_1", 1.0, 0.0, 5.0};
   RooRealVar mu2{"mu_2", "mu_2", 1.0, 0.0, 5.0};

   // Plain sums of histogram shapes, with no RooBinWidthFunctions.
   RooRealSumPdf model1{"model_1", "model_1", {hfSig1}, {mu1}};
   RooRealSumPdf model2{"model_2", "model_2", {hfSig2}, {mu2}};
   model1.setAttribute("BinnedLikelihood");
   model2.setAttribute("BinnedLikelihood");
   ...
   auto setParams = [&](bool alternative) {
      mu1.setVal(alternative ? 1.5 : 1.0);
      mu2.setVal(alternative ? 0.8 : 1.0);
   };
