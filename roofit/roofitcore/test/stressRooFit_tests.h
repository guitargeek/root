...
public:
   ...
   bool testCode() override
   {
      ...
      // of likelihood does not take any external definition of errors). Data with non-unit weights can only be
      // correctly fitted with a chi^2 fit (see rf602_chi2fit.C)
...
public:
   ...
   bool testCode() override
   {
      ...
      // in Y axis label (100 bins --> Events/0.4*Xaxis-dim) so that all bins represent a consistent density
      // distribution
...
public:
   ...
   bool testCode() override
   {
      ...
      // C o n s t r u c t   c o n d i t i o n a l   p r o d u c t   d e c a y _ d m ( d t | d t e r r ) * p d f ( d t e
      // r r )
      // ----------------------------------------------------------------------------------------------------------------------
...
public:
   ...
   bool testCode() override
   {
      ...
      // range named "signal"
      std::unique_ptr<RooAbsReal> igxy_sig{
         gxy.createIntegral(RooArgSet(x, y), NormSet(RooArgSet(x, y)), Range("signal"))};
...
public:
   ...
   bool testCode() override
   {
      ...
      // P l o t   d e c a y   d i s t r .   f o r   m i x e d   a n d   u n m i x e d   s l i c e   o f   m i x S t a t
      // e
      // ------------------------------------------------------------------------------------------------------------------
...
public:
   ...
   bool testCode() override
   {
      ...
      TH1 *hh_data_sel =
         data->createHistogram("hh_data_sel", x, Binning(8), YVar(y, Binning(8)), Cut("cut==cut::accept"));
...
public:
   ...
   bool testCode() override
   {
      ...
      RooBCPEffDecay bcp("bcp", "bcp", dt, tagFlav, tau, dm, w, CPeigen, absLambda, argLambda, effR, dw, tm,
...
public:
   ...
   bool testCode() override
   {
      ...
      RooMCStudy mcstudy{model, x, Binned(true), Silence(), Extended(), FitOptions(Save(true), PrintEvalErrors(0))};
