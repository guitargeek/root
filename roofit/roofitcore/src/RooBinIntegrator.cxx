   ...
   : RooAbsIntegrator(function), _useIntegrandLimits(true)
{
   ...
   for (UInt_t i = 0; i < _function->getDimension(); i++) {
      ...
      std::unique_ptr<std::list<double>> tmp{_function->binBoundaries(i)};
      if (!tmp) {
         oocoutW(nullptr, Integration)
            << "RooBinIntegrator::RooBinIntegrator WARNING: integrand provide no binning definition observable #" << i
            << " substituting default binning of " << _numBins << " bins" << std::endl;
         tmp = std::make_unique<std::list<double>>();
...
bool RooBinIntegrator::checkLimits() const
{
   ...
   for (UInt_t i = 0; i < _function->getDimension(); i++) {
      if (_xmax[i] <= _xmin[i]) {
         oocoutE(nullptr, Integration) << "RooBinIntegrator::checkLimits: bad range with min >= max (_xmin = "
                                       << _xmin[i] << " _xmax = " << _xmax[i] << ")" << std::endl;
         return false;
      }
