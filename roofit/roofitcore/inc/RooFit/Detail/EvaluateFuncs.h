...
inline double polyInterpValue(unsigned int i, double *polCoeff, double x)
{
                                            ...
                                            double paramVal, double total)
{
   if (code == 0) {
      ...
      // pice-wise log
      if (paramVal >= 0)
         return total * pow(high / nominal, +paramVal);
      else
         return total * pow(low / nominal, -paramVal);
   } else if (code == 2) {
      ...
      if (x >= boundary) {
         ...
         return total * polyInterpValue(i, polCoeff, x);
      }
   }
...
inline double flexibleInterpVarEvaluate(double *code, double *params, unsigned int paramSize,
                                        double *low, double *high, double nominal, double boundary, double * polCoeff)
{
   double total = nominal;
   for (std::size_t i = 0; i < paramSize; ++i) {
      total = flexibleInterpVarProcessParam(code[i], i, polCoeff, low[i], high[i], boundary, nominal, params[i],
                                            total);
   }
   if (total <= 0) {
