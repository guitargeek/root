...
void RooCruijff::doEval(RooFit::EvalContext &ctx) const
{
   RooBatchCompute::compute(
      ctx.config(this), RooBatchCompute::Cruijff, ctx.output(),
      {ctx.at(_x), ctx.at(_mu), ctx.at(_sigmaL), ctx.at(_sigmaR), ctx.at(_alphaL), ctx.at(_alphaR), ctx.at(_beta)});
}
