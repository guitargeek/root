...
namespace HistFactory{
  ...
  void HistoToWorkspaceFactoryFast::ConfigureWorkspaceForMeasurement( const std::string& ModelName, RooWorkspace* ws_single, Measurement& measurement ) {
   ...
   //throw hf_exc();
    ...
    if(poi_list.size()!=0){
      proto_config->GuessObsAndNuisance(*expData, RooMsgService::isActive(nullptr, RooFit::HistFactory, RooFit::INFO));
    }
   ...
   void HistoToWorkspaceFactoryFast::AddConstraintTerms(RooWorkspace* proto, Measurement & meas, string prefix,
    ...
    for(unsigned int i = 0; i < systList.size(); ++i) {
      ...
      if (meas.GetGammaSyst().count(sys.GetName()) > 0 ) {
         ...
         if (RooMsgService::isActive(nullptr, RooFit::HistFactory, RooFit::DEBUG)) {
           alphaOfBeta->Print("t");
           gamma->Print("t");
         }
...
//      string syst_x_expectedPrefix = "";
    ...
    if (RooMsgService::isActive(nullptr, RooFit::HistFactory, RooFit::INFO)) asymcalcPrintLevel = 1;
    if (RooMsgService::isActive(nullptr, RooFit::HistFactory, RooFit::DEBUG)) asymcalcPrintLevel = 2;
    ...
    if (RooMsgService::isActive(nullptr, RooFit::HistFactory, RooFit::INFO))
      proto->Print();
