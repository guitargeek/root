...
#include "Batches.cuh"
#define ROOBATCHCOMPUTE_BEGIN blockDim.x*blockIdx.x + threadIdx.x
#define ROOBATCHCOMPUTE_STEP  blockDim.x*gridDim.x
...
//~  __HOST__ __DEVICE__ void run(size_t begin, size_t batchSize, double * __restrict output, Tm M, Tm0 M0, Tc C, Tp P ) const
//~  {
  //~  for (size_t i=begin; i<batchSize; i+=step) {
...
__global__ void computeGaussian(Batches batches) 
{
  ...
  for (size_t i=ROOBATCHCOMPUTE_BEGIN; i<batches.getNEvents(); i+=ROOBATCHCOMPUTE_STEP) {
...
RooBatchComputeInterface::RooBatchComputeInterface()
  : computeFunctions{computeGaussian}
{
}
