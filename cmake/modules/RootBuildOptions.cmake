...
ROOT_BUILD_OPTION(builtin_cfitsio OFF "Build CFITSIO from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_clang ON "Build bundled copy of Clang (advanced option)")
...
ROOT_BUILD_OPTION(builtin_civetweb OFF "Build civetweb from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_fftw3 OFF "Build FFTW3 from an automatically downloaded source tarball (requires network) [GPL]")
ROOT_BUILD_OPTION(builtin_freetype OFF "Build freetype from an automatically downloaded source tarball (requires network)")
...
ROOT_BUILD_OPTION(builtin_llvm ON "Build bundled copy of LLVM (advanced option)")
...
ROOT_BUILD_OPTION(builtin_pcre OFF "Build PCRE2 from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_png OFF "Build libpng from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_tbb OFF "Build TBB from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_tiff OFF "Build libtiff from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_unuran OFF "Build unuran from an automatically downloaded source tarball (requires network) [GPL]")
ROOT_BUILD_OPTION(builtin_vdt OFF "Build VDT from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_xrootd OFF "Build XRootD from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_xxhash OFF "Build xxHash from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_zlib OFF "Build zlib from an automatically downloaded source tarball (requires network)")
ROOT_BUILD_OPTION(builtin_zstd OFF "Build zstd from an automatically downloaded source tarball (requires network)")
...
ROOT_BUILD_OPTION(clad_in_tree OFF "Build clad as a part of the ROOT project instead of as an external project, so that changing clad only triggers an incremental ROOT build (sources taken from -DCLAD_SOURCE_DIR=<clad_src_path>, by default <root_src>/clad)")
