 ...
 * All rights reserved.                                                  *
 *                                                                       *
 * For the licensing terms see $MyROOTSYS/LICENSE.                         *
 * For the list of contributors see $MyROOTSYS/README/CREDITS.             *
 *************************************************************************/
...
/// ~~~{.cpp}
/// using namespace MyROOT::VecOps;
/// const MyROOT::Math::PtEtaPhiMVector lv0 {15.5f, .3f, .1f, 105.65f},
///   lv1 {34.32f, 2.2f, 3.02f, 105.65f},
///   lv2 {12.95f, 1.32f, 2.2f, 105.65f};
/// RVec<MyROOT::Math::PtEtaPhiMVector> v {lv0, lv1, lv2};
/// auto v_sum_lv = Sum(v, MyROOT::Math::PtEtaPhiMVector());
/// v_sum_lv
/// // (MyROOT::Math::LorentzVector<MyROOT::Math::PtEtaPhiM4D<double> > &) (30.8489,2.46534,2.58947,361.084)
/// ~~~
template <typename T>
...
///   lv2 {12.95f, 1.32f, 2.2f, 105.65f};
/// RVec<MyROOT::Math::PtEtaPhiMVector> v {lv0, lv1, lv2};
/// auto v_mean_lv = Mean(v, MyROOT::Math::PtEtaPhiMVector());
/// v_mean_lv
/// // (MyROOT::Math::LorentzVector<MyROOT::Math::PtEtaPhiM4D<double> > &) (10.283,2.46534,2.58947,120.361)
/// ~~~
...
/// v_square
/// // (MyROOT::VecOps::RVec<float> &) { 2.00000f, 4.00000f, 8.00000f }
///
/// RVecF x({1.f, 2.f, 3.f});
/// RVecF y({4.f, 5.f, 6.f});
/// RVecF z({7.f, 8.f, 9.f});
...
/// v_mod
/// // (MyROOT::VecOps::RVec<float> &) { 8.12404f, 9.64365f, 11.2250f }
/// ~~~
...
auto Map(Args &&... args)
{
   ...
   */
   ...
   constexpr bool isRVec[]{MyROOT::Internal::VecOps::IsRVec<std::remove_cv_t<std::remove_reference_t<Args>>>::value...};
   static_assert(MyROOT::Internal::VecOps::All(isRVec, nArgs - 1),
                 "Map: the first N-1 arguments must be RVecs or references to RVecs");

   return MyROOT::Internal::VecOps::MapFromTuple(std::forward_as_tuple(args...),
...
/// // (MyROOT::VecOps::RVec<unsigned long> &) { 2, 0, 1 }
/// auto values = Take(v, sortIndices)
/// // (MyROOT::VecOps::RVec<double> &) { 1.0000000, 2.0000000, 3.0000000 }
/// ~~~
template <typename T>
...
/// // (MyROOT::VecOps::RVec<unsigned long> &) { 1, 0, 2 }
/// auto values = Take(v, sortIndices)
/// // (MyROOT::VecOps::RVec<double> &) { 3.0000000, 2.0000000, 1.0000000 }
/// ~~~
...
/// // (MyROOT::VecOps::RVec<unsigned long> &) { 3, 0, 2, 1 }
/// auto values = Take(v, sortIndices)
/// // (MyROOT::VecOps::RVec<double> &) { 1.0000000, 2.0000000, 2.0000000, 3.0000000 }
/// ~~~
template <typename T>
...
/// // (MyROOT::VecOps::RVec<unsigned long> &) { 1, 0, 2, 3 }
/// auto values = Take(v, sortIndices)
/// // (MyROOT::VecOps::RVec<double> &) { 3.0000000, 2.0000000, 2.0000000, 1.0000000 }
/// ~~~
template <typename T, typename Compare>
...
/// vTaken
/// // (MyROOT::VecOps::RVec<double>) { 2.0000000, 1.0000000 }
/// ~~~
...
/// v_reverse
/// // (MyROOT::VecOps::RVec<double> &) { 1.0000000, 3.0000000, 2.0000000 }
/// ~~~
template <typename T>
...
/// v_sorted
/// // (MyROOT::VecOps::RVec<MyROOT::VecOps::RVec<double> > &) { { 1.0000000, 4.0000000 }, { 2.0000000, 1.0000000 }, { 2.0000000, 4.0000000 }, { 3.0000000, 1.0000000 } }
/// ~~~
// clang-format off
template <typename T, typename Compare>
...
/// comb_idx
/// // (MyROOT::VecOps::RVec<MyROOT::VecOps::RVec<unsigned long> > &) { { 0, 0, 1, 1, 2, 2 }, { 0, 1, 0, 1, 0, 1 } }
/// ~~~
...
/// v_1
/// (MyROOT::VecOps::RVec<MyROOT::VecOps::RVec<unsigned long> > &) { { 0, 1, 2, 3 } }
/// auto v_2 = Combinations(v, 2);
/// v_2
/// (MyROOT::VecOps::RVec<MyROOT::VecOps::RVec<unsigned long> > &) { { 0, 0, 0, 1, 1, 2 }, { 1, 2, 3, 2, 3, 3 } }
/// auto v_3 = Combinations(v, 3);
/// v_3
/// (MyROOT::VecOps::RVec<MyROOT::VecOps::RVec<unsigned long> > &) { { 0, 0, 0, 1 }, { 1, 1, 2, 2 }, { 2, 3, 3, 3 } }
...
/// nonzero_idx
/// // (MyROOT::VecOps::RVec<unsigned long> &) { 0, 2, 4 }
/// ~~~
template <typename T>
...
/// v1_intersect_v2
/// // (MyROOT::VecOps::RVec<double> &) { 1.0000000, 2.0000000 }
/// ~~~
template <typename T>
...
/// auto if_c_v1_else_v2 = Where(c, v1, v2);
/// if_c_v1_else_v2
/// // (MyROOT::VecOps::RVec<double>) { 4.0000000, 2.0000000, 3.0000000 }
/// ~~~
template <typename T>
...
/// auto if_c_v1_else_v2 = Where(c, v1, v2);
/// if_c_v1_else_v2
/// // (MyROOT::VecOps::RVec<double>) { 1.0000000, 4.0000000, 4.0000000 }
/// ~~~
template <typename T>
...
/// auto if_c_v1_else_v2 = Where(c, v1, v2);
/// if_c_v1_else_v2
/// // (MyROOT::VecOps::RVec<double>) { 2.0000000, 4.0000000, 4.0000000 }
/// ~~~
template <typename T>
...
/// RVecI rvi {7, 8, 9};
/// Concatenate(rvf, rvi)
/// // (MyROOT::VecOps::RVec<float>) { 0.00000f, 1.00000f, 2.00000f, 7.00000f, 8.00000f, 9.00000f }
/// ~~~
...
/// auto fourVecs = Construct<MyROOT::Math::PtEtaPhiMVector>(pts, etas, phis, masses);
/// cout << fourVecs << endl;
...
{
   const auto size = ::MyROOT::Internal::VecOps::GetVectorsSize("Construct", args...);
   RVec<T> ret;
   ret.reserve(size);
   for (auto i = 0UL; i < size; ++i) {
...
// Allow to use RVec as MyROOT::RVec
using MyROOT::VecOps::RVec;

using RVecB = MyROOT::VecOps::RVec<bool>;
using RVecC = MyROOT::VecOps::RVec<char>;
using RVecD = MyROOT::VecOps::RVec<double>;
using RVecF = MyROOT::VecOps::RVec<float>;
using RVecI = MyROOT::VecOps::RVec<int>;
using RVecL = MyROOT::VecOps::RVec<long int>;
using RVecLL = MyROOT::VecOps::RVec<long long int>;
using RVecU = MyROOT::VecOps::RVec<unsigned int>;
using RVecUL = MyROOT::VecOps::RVec<unsigned long int>;
using RVecULL = MyROOT::VecOps::RVec<unsigned long long int>;
