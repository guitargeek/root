...
class MnFcnCaller {
public:
   MnFcnCaller(const MnFcn &mfcn) : _mfcn{mfcn}, _isUserFcn{static_cast<bool>(dynamic_cast<MnUserFcn const *>(&mfcn))}
   {
      if (!_isUserFcn)
         return;

      MnUserTransformation const &transform = static_cast<MnUserFcn const &>(_mfcn).transform();

      // get first initial values of parameter (in case some one is fixed)
      _vpar.assign(transform.InitialParValues().begin(), transform.InitialParValues().end());
   }
   ...
   double operator()(const MnAlgebraicVector &v)
   {
      ...
      bool firstCall = _lastInput.size() != v.size();

      _lastInput.resize(v.size());

      for (unsigned int i = 0; i < v.size(); i++) {
         if (firstCall || _lastInput[i] != v(i)) {
            _vpar[transform.ExtOfInt(i)] = transform.Int2ext(i, v(i));
            _lastInput[i] = v(i);
         }
      }

      return static_cast<MnUserFcn const &>(_mfcn).call(_vpar);
   }

private:
   MnFcn const &_mfcn;
   bool _isUserFcn = false;
   std::vector<double> _lastInput;
   std::vector<double> _vpar;
};
